...
export class TensorflowComponent implements OnInit {
  ...
  constructor() { }
   ...
   async ngOnInit() {
     this.model = await tf.loadLayersModel('https://raw.githubusercontent.com/jamescalam/meditations_ai/master/incl_models/LSTM_e256_sl100_0414/model_js/model.json');  // load model
     // also load the character to index mappings
     this.char2idx = await $.getJSON('https://raw.githubusercontent.com/jamescalam/meditations_ai/master/incl_models/LSTM_e256_sl100_0414/char2idx.json', function(json:any) {
 ...
 async function convertIdx(c2i:any) {
   let idx2char: string[];
 ...
 function getNum(c2i:any, text: string) {
   let idxArray: number[] = [];
